...
async function loginAndGetCookies() {
  ...
  await page.goto(INSTAGRAM_LOGIN_URL, { waitUntil: "networkidle" });
  ...
  const cookieString = cookies.map((c) => `${c.name}=${c.value}`).join("; ");
  console.log("🍪 Cookies saved and ready!");
  return cookieString;
}
...
/** ✅ جلب بوستات المستخدم من GraphQL Web API */
async function getUserPosts(userId, cookies, count = 12) {
  const queryHash = "69cba40317214236af40e7efa697781d"; // ثابت ل timeline_media
  const variables = {
    id: userId,
    first: count,
  };
...
(async () => {
  try {
    ...
    const result = { profile: profileData, posts };
    fs.writeFileSync(`${TARGET_USER}_data.json`, JSON.stringify(result, null, 2));
